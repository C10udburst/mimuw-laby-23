...
ssize_t PPCB::Packet::size() const {
    switch (type) {
        case PacketType::CONN: return sizeof(PacketType) + sizeof(session_id) + sizeof(conn);
        case PacketType::CONNACC: return sizeof(PacketType) + sizeof(session_id) + sizeof(acc);
        case PacketType::CONRJT: return sizeof(PacketType) + sizeof(session_id) + sizeof(rjt);
        case PacketType::DATA: return sizeof(PacketType) + sizeof(session_id) + sizeof(data.packet_id) +
                                      sizeof(data.data_length) + ntohl(data.data_length);
        case PacketType::ACC: return sizeof(PacketType) + sizeof(session_id) + sizeof(acc);
        case PacketType::RJT: return sizeof(PacketType) + sizeof(session_id) + sizeof(rjt);
        case PacketType::RCVD: return sizeof(PacketType) + sizeof(session_id);
...
void PPCB::Packet::validate(ssize_t len) const {
    ...
    if (type == PacketType::DATA) {
        ...
        if (ntohl(data.data_length) >
            MAX_PACKET_SIZE - sizeof(PacketType) - sizeof(session_id) - sizeof(data.packet_id) -
            sizeof(data.data_length)) {
...
void PPCB::Packet::print() const {
    ...
    switch (type) {
        ...
        case PPCB::PacketType::RCVD:
            ...
            break;
        default:
            std::cout << "UNKNOWN:" << static_cast<int>(type);
            break;
    }
    ...
    if (type == PacketType::CONN) {
        ...
        std::cout << ", packet_id=" << ntohll(data.packet_id) << ", data_length=" << ntohl(data.data_length)
                  << ", data=\"";
