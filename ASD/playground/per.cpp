...
struct DP {
    uint64_t bez3[3]; // bez3[i] - maks wart. bez podciagu 3 tz na koncu jest i wybranych
    uint64_t z3[4]; // z3[i] - maks wart. z podciagiem 3 tz na koncu jest i wybranych
};
...
int main(int argc, char **argv) {
    ...
    struct DP old = {0, 0, 0, 0, 0, 0, 0};
    struct DP cur = {0, 0, 0, 0, 0, 0, 0};

    uint64_t wart;
    while (n--) {
        cin >> wart;

        cur.bez3[0] = max(max(old.bez3[0], old.bez3[1]), old.bez3[2]);
        cur.bez3[1] = old.bez3[0] + wart;
        cur.bez3[2] = old.bez3[1] + wart;

        cur.z3[0] = max(max(old.z3[0], old.z3[1]), max(old.z3[2], old.z3[3]));
        cur.z3[1] = old.z3[0] + wart;
        cur.z3[2] = old.z3[1] + wart;
        cur.z3[3] = old.bez3[2] + wart;

        swap(old, cur);
    }

    uint64_t max_v = 0;
    for (uint64_t i : old.bez3)
        max_v = max(max_v, i);
    for (uint64_t i : old.z3)
        max_v = max(max_v, i);
