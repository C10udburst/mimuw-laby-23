...
def run(protocol):
    ...
    server = subprocess.Popen(
        ["./run", "server", "bash", "-c", f"timeout 300s /work/ppcbs {server_prot} 21370 > /dev/null"],
        stdin=subprocess.PIPE, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
    client = subprocess.Popen(
        ["./run", "client1", "bash", "-c", f"cat /work/bigfile.bin | timeout 300s /work/ppcbc {protocol} server 21370"],
        stdin=subprocess.PIPE, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
    try:
        client.wait()
    except:
