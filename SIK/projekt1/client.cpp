...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "tcp") == 0) {
        return tcpClient(send_address, cleaner);
    } else if (strcmp(argv[1], "udp") == 0) {
        return udpClient(send_address, cleaner);
    } else if (strcmp(argv[1], "udpr") == 0) {
        return udprClient(send_address, cleaner);
    } else {
...
int udpClient(struct sockaddr_in send_address, Cleaner &cleaner) {
...

    {
        ...
        ssize_t sent = sendto(sockfd, &packet, packet.size(), 0, (struct sockaddr *) &send_address, sizeof(send_address));
        if (sent != packet.size()) {
    ...
    { // receive RCVD
        socklen_t len = sizeof(send_address);
        ssize_t recv_len = recvfrom(sockfd, buffer, PPCB::MAX_PACKET_SIZE, 0, (struct sockaddr *) &send_address, &len);
        if (recv_len < 0) {
